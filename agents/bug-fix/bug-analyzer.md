...
2. **이전 버그 기록 확인**: `.claude/bug-report/{feature}/` 내 기존 report.md들을 읽어:
   - 같은 feature에서 이전에 발생한 유사 버그 확인
   - 이전 수정 시도에서 실패한 접근법 파악 (동일 실수 반복 방지)
   - 성공한 수정 패턴 참고
   - 분석 리포트의 "이전 기록 참고" 필드에 발견 내용 기술
3. **관련 코드 탐색**: Glob, Grep으로 연관 파일 검색
4. **기존 테스트 확인**: 해당 기능의 테스트 존재 여부

## 산출물

### 분석 리포트 (report.md의 "분석" 섹션에 기록)
